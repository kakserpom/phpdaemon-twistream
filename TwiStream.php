...
class TwiStream extends AppInstance {
	...
	public function onReady() {
		...
		if ($this->WS = Daemon::$appResolver->getInstanceByAppName('WebSocketServer')) {
			$this->WS->addRoute('TwiStream',function ($client) use ($appInstance)
			{
...
class TwiStreamWebSocketSession extends WebSocketRoute { 
	...
	public function onFrame($data, $type) {
		...
		if (!isset($o['cmd']) || !is_string($o['cmd'])) {return;}
		if ($o['cmd'] === 'subscribe') {
			$r = new stdClass;
			$r->attrs = $o['attrs'];
	...
	public function onFinish() {
		...
		while ($id = array_pop($this->requests)) {
		 ...
		 if (!isset(Daemon::$worker->queue[$id])) {continue;}
	...
	public function init() {
		try {
			...
			$this->stream->
				...
				setRequest($this)->
				enable()->
				write("POST /1/statuses/filter.json HTTP/1.1\r\n"
							."Connection: close\r\n"
							."Content-type: application/x-www-form-urlencoded\r\n"
							."Authorization: basic ".base64_encode($this->appInstance->config->user->value.':'.$this->appInstance->config->password->value)."\r\n"
